...
for i =1:length(hObj.UserData.Selection)
    ...
    if exist(fullfile(DIR_SAVE,FILES(ii).gfus,'Time_Groups.mat'),'file')
        load(fullfile(DIR_SAVE,FILES(ii).gfus,'Time_Groups.mat'),'TimeGroups_name');
...
for i =1:length(hObj.UserData.Selection)
    ii = hObj.UserData.Selection(i);
    if exist(fullfile(DIR_SAVE,FILES(ii).gfus,'Time_Tags.mat'),'file')
        load(fullfile(DIR_SAVE,FILES(ii).gfus,'Time_Tags.mat'),'TimeTags_cell');
...
if flag_gload
    load_graphicdata(fullfile(DIR_SAVE,FILES(CUR_FILE).gfus),myhandles);
end
if flag_dload
    load_global_image(fullfile(DIR_SAVE,FILES(CUR_FILE).gfus),myhandles.CenterPanelPopup.Value);
...
for i = 1:length(ind_files)
    ...
    %File Selection
    ii=ind_files(i);
    file_name = myhandles.FileSelectPopup.String(ii,:);
    ...
    load(fullfile(DIR_SAVE,FILES(ii).gfus,'Time_Reference.mat'),'n_burst');
        ...
        %try
            switch process_name
                case 'Compute Normalized Movie'
                    success = compute_normalizedmovie(fullfile(DIR_SAVE,FILES(ii).gfus),myhandles);
                ...
                case 'Import Spikoscope Regions'
                    success = import_regions(SEED_REGION,FILES(ii).spiko,fullfile(DIR_SAVE,FILES(ii).gfus));
                    
                case 'Import Spikoscope Traces'
                    success = import_traces(fullfile(SEED,FILES(ii).parent,FILES(ii).spiko),fullfile(DIR_SAVE,FILES(ii).gfus));
