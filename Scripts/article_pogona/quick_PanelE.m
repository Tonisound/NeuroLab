...
list_variables_1 = {'FirstTime';'MaxTime';'HalfTime';'MinTime';};
all_data_1 = NaN(length(list_animals),length(list_tags),max(all_counters(:)),length(list_variables_1));
for k4=1:length(list_variables_1)
    for k2=1:length(list_tags)
        for k1=1:length(list_animals)
            if ~isempty(S(k1,k2,1).all_locs_regions)
                temp = permute(S(k1,k2,1).all_locs_regions(:,k4),[3,2,1]);
                all_data_1(k1,k2,1:length(temp),k4) = temp;
            end
        end
    end
end

list_variables_2 = {'FirstPeak';'MaxPeak';'HalfPeak';'MinPeak';};
all_data_2 = NaN(length(list_animals),length(list_tags),max(all_counters(:)),length(list_variables_2));
for k4=1:length(list_variables_2)
    for k2=1:length(list_tags)
        for k1=1:length(list_animals)
            if ~isempty(S(k1,k2,1).all_pks_regions)
                temp = permute(S(k1,k2,1).all_pks_regions(:,k4),[3,2,1]);
                all_data_2(k1,k2,1:length(temp),k4) = temp;
            end
        end
    end
end
...
for k2=1:length(list_tags)
    for k1=1:length(list_animals)
        ...
        for i=1:size(S(k1,k2,1).all_r,1)
            line('XData',lags,'YData',S(k1,k2,1).all_r(i,:),'LineWidth',.25,'Parent',ax,'Color',g_colors(k1,:));
...
%         line('XData',lags,'YData',mean(S(k1,k2,1).all_r,1,'omitnan')-sem(S(k1,k2,1).all_r,1,'omitnan'),'LineWidth',.5,'Parent',ax,'Color',[.5 .5 .5]);
        ...
        patch('XData',patch_xdata,'YData',patch_ydata,'EdgeColor','none','Parent',ax,'FaceColor',g_colors(k1,:),'FaceAlpha',.5);
...
for k4 = 1:length(list_variables_1)
    ...
    for k2=1:length(list_tags)
        ...
        scatter(scatter_xx(:),scatter_yy(:),'Color',g_colors(k2,:));
    end
    ax.Title.String = char(list_variables_1(k4));
    grid(ax,"on");
    l = legend(list_tags);
...
for k2=1:length(list_tags)
    scatter_xx = scatter_x(:,k2,:);
    scatter_yy = all_data_1(:,k2,:,1)-all_data_1(:,k2,:,3);
    scatter(scatter_xx(:),scatter_yy(:),'Color',g_colors(k2,:));  
end
ax.Title.String = 'First Time Diff';
grid(ax,"on");
...
for k2=1:length(list_tags)
    scatter_xx = scatter_x(:,k2,:);
    scatter_yy = all_data_1(:,k2,:,2)-all_data_1(:,k2,:,4);
    scatter(scatter_xx(:),scatter_yy(:),'Color',g_colors(k2,:));  
end
ax.Title.String = 'Max Time Diff';
grid(ax,"on");
...
for k2=1:length(list_tags)
    scatter_xx = scatter_x(:,k2,:);
    scatter_yy = all_data_2(:,k2,:,1)-all_data_2(:,k2,:,3);
    scatter(scatter_xx(:),scatter_yy(:),'Color',g_colors(k2,:));  
end
ax.Title.String = 'First Peak Diff';
grid(ax,"on");
...
for k2=1:length(list_tags)
    scatter_xx = scatter_x(:,k2,:);
    scatter_yy = all_data_2(:,k2,:,2)-all_data_2(:,k2,:,4);
    scatter(scatter_xx(:),scatter_yy(:),'Color',g_colors(k2,:));  
end
ax.Title.String = 'Max Peak Diff';
grid(ax,"on");
