...
% norm_output_flag - set to 1 to normalize from the zeros lag value, 2
% biased normalisation, 3 unbiased normalisation
...
if nargin==7
    ...
    if isempty(find(norm_output_flag==[0 1 2 3]))
        error('norm_output_flag must be set to either 0, 1 ,2 or 3.')
    end
...
%--------------------------------------------------------------------------
if norm_output_flag==1 %normalisation from the zero lag
   ...
   xcl=xcl./(xcl(:,Nlag_max+1)*ones(1,2*Nlag_max+1));
elseif norm_output_flag==2 %biased normalisation normalisation by 1/Lwin
    xcl=xcl./(Lwin);
elseif norm_output_flag==3 %unibiased normalisation
    % Scales the raw correlation by 1/(Lwin-abs(lags)).
     L = (size(xcl,2) - 1)/2;
    scaleUnbiased = (Lwin - abs(-L:L)).';
   % scaleUnbiased(scaleUnbiased <= 0) = 1;

   xcl=xcl./repmat(scaleUnbiased',size(xcl,1),1);
end