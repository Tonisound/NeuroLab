% Panel D - Figure fUS-EPI - Guyon et al. 2026
% Stim vs Light-Control
...
%         permute(d_3stim.Y2q_evt_normalized(ind_stim_region,:,:),[3 2 1])];
    ...
    imagesc('XData',t_bins_fus,'YData',1:n_stim_trials,'CData',all_stim_trials,'Parent',ax11);
    % Stims
    for i = 1:n_stim_trials
    ...
    set(ax11,'YTick',1:n_stim_trials,'YTickLabel',1:n_stim_trials);
    set(ax11,'XTick',0:10:t_end_auc);
    colorbar(ax11);
    ax11.Title.String = 'Ipsi (All trials)';
    set(ax11,'XLim',[-10 t_end_auc],'YLim',[.5 n_stim_trials+.5]);
    ...
    ax13.Title.String = 'Light Control (All trials)';
    set(ax13,'XLim',[-10 t_end_auc],'YLim',[.5 n_control_trials+.5]);
    ...
    py1_data = [mean(YData1,1)+(std(YData1,[],1)/sqrt(n_stim_trials)),fliplr(mean(YData1,1)-(std(YData1,[],1)/sqrt(n_stim_trials)))]';
    patch('XData',px1_data,'YData',py1_data,'FaceColor',g_colors(1,:),'EdgeColor','none','Parent',ax12,'FaceAlpha',.25);

    YData2 = all_control_trials(1:n_control_trials,:);
    l2 = line('XData',t_bins_fus,'YData',mean(YData2,1),...
        'Color',g_colors(2,:),'Parent',ax12,'Tag','Light-Control-100um');
    px2_data = [t_bins_fus;flipud(t_bins_fus)];
    py2_data = [mean(YData2,1)+(std(YData2,[],1)/sqrt(n_control_trials)),fliplr(mean(YData2,1)-(std(YData2,[],1)/sqrt(n_control_trials)))]';
    ...
    set(ax14,'XLim',[.5 n_control_trials+.5]);
    set(ax14,'XTick',1:n_control_trials,'XTickLabel',leg_labels);
    grid(ax14,'on');

    all_bdata = [all_bdata;bdata];

    picname = f1.Name;
    saveas(f1,fullfile(dir_save,"Stim-vs-LightControl",picname),ImageSaveFormat);
