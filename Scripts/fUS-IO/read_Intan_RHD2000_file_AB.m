function S = read_Intan_RHD2000_file_AB(path)
...
%     uigetfile('/media/hobbes/DataMOBs204/Ephys/*.rhd', 'Select an RHD2000 Data File', 'MultiSelect', 'off');

% Read most recent file automatically.
% path = '/media/hobbes/DataMOBs204/Ephys';
% d = dir(fullfile(path,'*.rhd'));
% file = d(end).name;

tic;
% filename = fullfile(path,file);
filename = fullfile(path,'info.rhd');
...
if (num_board_dig_out_channels > 0)
    ...
    S.board_dig_out_channels = board_dig_out_channels;
    if (data_present)
        % move_to_base_workspace(board_dig_out_data);
        S.board_dig_out_data = board_dig_out_data;
        % move_to_base_workspace(t_dig);
        S.t_dig = t_dig;
    end
end
if (num_temp_sensor_channels > 0)
    if (data_present)
        ...
        S.temp_sensor_data = temp_sensor_data;
        % move_to_base_workspace(t_temp_sensor);
        S.t_temp_sensor = t_temp_sensor;
    end
end
