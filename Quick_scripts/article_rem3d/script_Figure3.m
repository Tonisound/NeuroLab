...
S = struct('reference','','region','','recording','',...
    'r_max',[],'t_max',[],'x_max',[],...
    'r_min',[],'t_min',[],'x_min',[],...
    'r_abs',[],'t_abs',[],'x_abs',[]);...    'x_',[],'RT_pattern',[]
...
switch str
    case 'Rmax'
        f.Name = strcat(fName,'-Rmax');
    case 'Tmax'
        f.Name = strcat(fName,'-Tmax');
    case 'Xmax'
        f.Name = strcat(fName,'-Xmax');
end
f.Renderer = 'Painters';
...
% 
% % Ax3
% b3 = barh(diag(tt_data2(ind_sorted)),'stacked','Parent',ax3);
% for i=1:length(b3)
%     %bar color
%     b3(i).FaceColor = f_colors(ind_sorted(i),:);
%     b3(i).EdgeColor = 'k';
%     b3(i).LineWidth = .1;
%     text(b3(i).YData(i),b3(i).XData(i),sprintf('%.2f',b3(i).YData(i)),'Parent',ax3);
% end
% % Axis limits
% ax3.YTick = 1:n_bars;
% ax3.YTickLabel = list_regions(ind_sorted);
% ax3.Title.String = 'PHASIC/TONIC';
% grid(ax3,'on');
% 
...
switch str
    case 'Rmax'
        fullname = fullfile(folder_save,strcat(fName,'-Rmax.pdf'));
    case 'Tmax'
        fullname = fullfile(folder_save,strcat(fName,'-Tmax.pdf'));
    case 'Xmax'
        fullname = fullfile(folder_save,strcat(fName,'-Xmax.pdf'));
end
saveas(f,fullname);
