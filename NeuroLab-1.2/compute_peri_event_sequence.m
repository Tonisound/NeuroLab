...
%             '[NREM]Ripples-Merged-Burst-Single[1.00sec]';...
...
%             'Y3q_evt_normalized','-v7.3'); % ,'Y3q_evt_normalized_int'
        Y3q_evt_normalized_reshaped = reshape(Y3q_evt_normalized,[size(IM,1) size(IM,2) length(t_bins_fus) n_events]);
        filename_save_2 = sprintf(strcat('%s_PeriEvent_AllEvents_normalized.mat'),event_name);
        save(fullfile(save_dir,filename_save_2),'Params',...
            'all_labels_channels','t_bins_lfp',...
            'Xq_evt_lfp_','Y0q_evt_','Y1q_evt_',...
            'all_labels_regions','t_bins_fus',...
            'Xq_evt_fus_','Y2q_evt_normalized',...
            'Y3q_evt_normalized_reshaped','-v7.3');
        fprintf('Data saved [%s].\n',fullfile(save_dir,filename_save_2));
        filename_save_3 = sprintf(strcat('%s_PeriEvent_AllEvents_raw.mat'),event_name);
        Y3q_evt_raw_reshaped = reshape(Y3q_evt_,[size(IM,1) size(IM,2) length(t_bins_fus) n_events]);
        save(fullfile(save_dir,filename_save_3),'Params',...
            'all_labels_channels','t_bins_lfp',...
            'Xq_evt_lfp_','Y0q_evt_','Y1q_evt_',...
            'all_labels_regions','t_bins_fus',...
            'Xq_evt_fus_','Y2q_evt_normalized',...
            'Y3q_evt_raw_reshaped','-v7.3');
        fprintf('Data saved [%s].\n',fullfile(save_dir,filename_save_3));
