    ...
    if exist(fullfile(folder_dest,strcat(event_name,'.mat')),'file')
        ...
        for i=1:length(all_files)
            ...
            for j=1:length(dd)
                ...
                m_z=mean(data_traces.Yraw_evt_(ind_z1:ind_z2,:));
                m_z_mat = repmat(m_z,[length(t_bins_lfp) 1]);
                std_z=std(data_traces.Yraw_evt_(ind_z1:ind_z2,:));
                std_z_mat = repmat(std_z,[length(t_bins_lfp) 1]);
                S(i).Yraw_evt_zscored = (data_traces.Yraw_evt_-m_z_mat)./std_z_mat;
                %             S(i).Yraw_evt_zscored = zscore(data_traces.Yraw_evt_);
            ...
            for j=1:length(dd)
                ...
                S(i).density_events = data_seq.Params.density_events;
...
%                     copyfile(fullfile(dd(k).folder,dd(k).name),fullfile(dd_dest,dd(k).name));
                    copyfile(fullfile(dd(k).folder,dd(k).name),fullfile(dd_dest,strcat(event_name,'-',dd(k).name)));
            ...
            for jj=1:length(unique_planes)
                ...
                for i=1:length(f_axes)
                    ...
                    for ii=1:S_animal_plane(i).n_events
                        ...
                        %                     l=line('XData',S_animal_plane(i).t_bins_lfp,'YData',S_animal_plane(i).Yraw_evt_zscored(:,ii),'Color','k','LineWidth',.1,'Parent',ax);
                        l.Color(4)=.5;
                    end
                    line('XData',S_animal_plane(i).t_bins_lfp,'YData',mean(S_animal_plane(i).Yraw_evt_,2,'omitnan'),'Color','r','LineWidth',2,'Parent',ax);
                    %                 line('XData',S_animal_plane(i).t_bins_lfp,'YData',mean(S_animal_plane(i).Yraw_evt_zscored,2,'omitnan'),'Color','r','LineWidth',2,'Parent',ax);
                ...
                for i=1:length(f_axes)
                    ...
                    n_iqr = 3;
                    Yraw_evt = S_animal_plane(i).Yraw_evt_zscored(:);
                    data_iqr = Yraw_evt(~isnan(Yraw_evt));
                    ax.YLim = [median(data_iqr(:))-n_iqr*iqr(data_iqr(:)),median(data_iqr(:))+n_iqr*iqr(data_iqr(:))];
                    ax.XLim = [-.1 .1];
                    set(ax,'XTick',[],'XTickLabel',[],'YTick',[],'YTickLabel',[]);
