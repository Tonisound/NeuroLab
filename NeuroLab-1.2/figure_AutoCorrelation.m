...
if val==1
    ...
    for i=1:length(l_reg)
        list_regions = [list_regions;{l_reg(i).UserData.Name}];
    end
    if ~isempty(list_regions)
        [ind_regions,ok] = listdlg('PromptString','Select Regions to display','SelectionMode','multiple',...
        'ListString',list_regions,'InitialValue',1:length(list_regions),'ListSize',[400 500]);
        str_regions = list_regions(ind_regions);
    end
end

% Restricting to selected regions 
if ~isempty(str_regions) && ~isempty(l_reg)
...
%         str_group_regions = [str_group_regions;{l_group(i).UserData.Name}];
%     end
%     if ~isempty(str_group_regions)
%     end
% end


% Restricting to selected groups 
